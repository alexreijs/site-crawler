...
function onResourceRequested(requestData, networkRequest) {
	if (!/^data:/.test(requestData.url))
		exportLists.resources.resources[requestData.id] = {"url": requestData.url, "requested": requestData.time.getTime() - timestamp, "completed": 0, "failed": 0, "party" : {"party" : "", "type" : ""}};
};
...
function onResourceReceived(response) {
	if (/3[0-9]{2}/.test(response.status))				// Redirect
		...
		exportLists.resources.resources[response.id]['party'] = detectParty.detectParty(2, '', response.url, '');
	}
};
