...
class RuleRepository {
	...
	$operator = $filter["operator"];
        ...
        $sql = "SELECT * FROM rules WHERE location LIKE :location AND name like :name AND (:company_id = 0 OR company_id = :company_id) AND (:type = 0 or type = :type) AND (:consent_type_id = 0 OR consent_type_id = :consent_type_id) AND (:operator = 0 OR operator = :operator)";
    ...
    public function insert($data) {
        $sql = "INSERT INTO rules (type, priority, location, name, company_id, consent_type_id, operator) VALUES (:type, :priority, :location, :name, :company_id, :consent_type_id, :operator)";
        $q = $this->db->prepare($sql);
    ...
    public function update($data) {
        $sql = "UPDATE rules SET type = :type, priority = :priority, location = :location, name = :name, company_id = :company_id, consent_type_id = :consent_type_id, operator = :operator WHERE id = :id";
        $q = $this->db->prepare($sql);
        $q->bindParam(":type", $data["type"]);
        $q->bindParam(":priority", $data["priority"]);
        $q->bindParam(":location", $data["location"]);
